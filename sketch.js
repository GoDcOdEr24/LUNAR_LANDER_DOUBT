...
function setup() {
  ...
  ground = createSprite(500, 770, 1000, 20);
  ground.addImage(lava)
  ground.scale = 0.74
  p = createSprite(680, 460, 50, 30)
  p.addImage(pimg)
  p.scale = 0.5
  rectMode(CENTER);
  textSize(15);
...
function draw() {
  ...
  pop();
  var d = dist(lander.position.x,lander.position.y,p.position.x,p.position.y)
  if(d <= 30 && (vy<2 && vy>-2)&& (vx<2 && vx >-2)){
  ...
  p.depth = lander.depth 
lander.depth = p.depth+1
  //fall down
  vy += g;
